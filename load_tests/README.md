...
(`LOAD_TEST_SELLER_EMAIL` / `LOAD_TEST_MANAGER_EMAIL`, or the older
`*_USERNAME` vars). A value with `@` is sent as email; otherwise as username.
After email-login is on stage, set the real emails, not `alikhan` / `seller`.
Do this only with a low spawn rate: the login endpoint is intentionally rate limited.
