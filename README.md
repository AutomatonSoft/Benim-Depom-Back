...
| `GET` | `/types/` | авторизован | Активные типы товара. |
| `GET/PATCH/DELETE` | `/categories/{id}/` | manager/admin | Управлять категорией. |
| `GET/PATCH/DELETE` | `/product-types/{id}/` | manager/admin | Управлять конкретным типом товара. |
...
| --- | --- | --- | --- |
| `product_type` | да | Конкретный вид товара. Используется в AI-сервисе как тип (`produktart`) и позднее будет маппиться в типы маркетплейсов. | `Chair` |
...
Путь `/catalog/types/` — это просто короткое API-название для справочника `ProductType`; отдельного третьего поля «type» у товара нет.
...
При успешном approve товар получает ровно два EAN: один с аккаунтом `jv`, второй — с `xl`. До одобрения коды не резервируются вообще. Seller не получает эти коды в API-ответах; их видят только manager/admin. Если в одном из пулов нет свободного кода, approve вернёт ошибку и товар останется `submitted`.
...
5. Загрузить `/catalog/types/` и при необходимости `/catalog/categories/` для формы товара.
